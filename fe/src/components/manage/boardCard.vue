<template>
  <v-card>
    <template v-if="!edit">
      ...
      <v-divider light></v-divider>
      <v-card-actions>
        <v-btn color="orange" text @click="modeChange(board)">수정</v-btn>
        <v-btn color="error" text @click="ca=true">삭제</v-btn>
      </v-card-actions>
    </template>
    ...
    <v-card-text v-if="ma.act">
      <v-alert v-model="ma.act" :type="ma.type" dismissaible>{{ ma.msg }}</v-alert>
    </v-card-text>
  </v-card>
</template>
...
export default {
  ...
  methods: {
    ...
    mod (board) {
      if (board.name === this.form.name && board.rmk === this.form.rmk && board.lv === this.form.lv) return this.pop('번경한 것이 없습니다.', 'warning')
